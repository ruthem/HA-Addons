...
WORKSPACE_PATH=$(jq -r '.workspace_path // "/config/openclaw"' "$OPTIONS_FILE")
CONFIG_PATH_OPTION=$(jq -r '.config_path // empty' "$OPTIONS_FILE")

# Set OPENCLAW_CONFIG_PATH early so it can be used throughout the script
OPENCLAW_CONFIG_PATH="/config/.openclaw/openclaw.json"
if [ -n "$CONFIG_PATH_OPTION" ]; then
    # If it's a relative path, make it relative to /config (addon_config) or use as is if absolute
    if [[ "$CONFIG_PATH_OPTION" != /* ]]; then
        OPENCLAW_CONFIG_PATH="/config/$CONFIG_PATH_OPTION"
    else
        OPENCLAW_CONFIG_PATH="$CONFIG_PATH_OPTION"
    fi
fi
...
export OPENCLAW_CONFIG_DIR=$(dirname "$OPENCLAW_CONFIG_PATH")
export OPENCLAW_WORKSPACE_DIR="$WORKSPACE_PATH"
export XDG_CONFIG_HOME=/config

mkdir -p "$OPENCLAW_CONFIG_DIR" "${OPENCLAW_CONFIG_DIR}/identity" "$WORKSPACE_PATH" /config/keys /config/secrets
...
PERSISTENT_SKILLS_DIR="${OPENCLAW_CONFIG_DIR}/skills"
...
mkdir -p "$OPENCLAW_CONFIG_DIR/agents" || true

# ------------------------------------------------------------------------------
# SINGLE-INSTANCE GUARD (prevents multiple gateway runs racing each other)
# ------------------------------------------------------------------------------
STARTUP_LOCK="$OPENCLAW_CONFIG_DIR/gateway.start.lock"
exec 9>"$STARTUP_LOCK"
if ! flock -n 9; then
...
workspace_path = os.environ.get('OPENCLAW_WORKSPACE_DIR', '/config/clawd')
...
print(f"INFO: Wrote minimal OpenClaw config to {cfg_path}")
PY
fi
...
if [ "$AUTO_CONFIGURE_MCP" = "true" ] && [ -n "$HA_TOKEN" ]; then
  if command -v mcporter >/dev/null 2>&1; then
    ...
    MCP_FLAG="$OPENCLAW_CONFIG_DIR/.mcp_ha_configured"
...
start_openclaw_runtime() {
  echo "Starting OpenClaw Gateway runtime (openclaw)..."
